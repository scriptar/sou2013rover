...
#define NUM_ROGOVARS		10

typedef enum {NT_UNKNOWN, NT_CONTROL, NT_OP, NT_NUM, NT_VAR, NT_SENSOR, NT_FUNC, NT_LIST_START, NT_LIST_END} NodeType;
typedef enum {DT_UNKNOWN, DT_INT, DT_DBL} DataType;
typedef enum {SENSOR_UNKNOWN, SENSOR_X, SENSOR_Y, SENSOR_HEADING, SENSOR_PING_FRONT, SENSOR_PING_REAR} SensorType;
...
typedef struct primitive {
    ...
    int numargs;
    int priority;
    TNODE *(*prim_function) (TNODE *);
} PRIMTYPE;

extern double rogo_vars[];
extern void forward(int);
extern void reverse(int);
